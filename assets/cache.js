...
window.onload = async () => {
    const filesRequest = await fetch(buildPath('cache/files'));
    const filesResponse = await filesRequest.json();
    
    const files = ['https://unpkg.com/html5-qrcode'];
    const pages = ['card', 'document', 'documents', 'id', 'more', 'pesel', 'qr', 'scan', 'services', 'shortcuts', 'show'];

    pages.forEach((page) => {
        files.push(buildHtmlPath(page) + '?' + params);
    })

    filesResponse.files.forEach((file) => {
        files.push(buildPath(file));
    })
    ...
    navigator.serviceWorker.register(buildPath('worker.js'));
}
